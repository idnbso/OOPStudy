    ...
    public T this[int index]
    {
        get
        {
            if (index < 0 || index >= Count)
            {
                ...
                "Index cannot be less than the index of the first element of the deque or greater than the index of the last element in the deque.");
            }

            if (front.Count - frontDeleted > index)
            {
                return front[front.Count - 1 - index];
            }
            else
            {
                return back[index - front.Count + frontDeleted];
            }
        }
    }
}
